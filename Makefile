...
build: ## 构建 debug 版本 (server)
	@echo "$(GREEN)构建 debug 版本...$(NC)"
	$(CARGO) build -p $(PACKAGE)

build-release: ## 构建 release 版本 (server)
	@echo "$(GREEN)构建 release 版本...$(NC)"
	$(CARGO) build --release -p $(PACKAGE)

build-agent: build ## 构建 server (debug)

build-agent-release: build-release ## 构建 server (release)

build-ring: ## 使用 ring 密码库构建 release 版本
	@echo "$(GREEN)使用 ring 构建 release 版本...$(NC)"
	$(CARGO) build --release -p $(PACKAGE) --no-default-features --features ring

build-jemalloc: ## 启用 JEMalloc 构建 release 版本
	@echo "$(GREEN)启用 JEMalloc 构建 release 版本...$(NC)"
	$(CARGO) build --release -p $(PACKAGE) --features jemallocator
...
test-agent: ## 运行 server 测试
	@echo "$(GREEN)运行 server 测试...$(NC)"
...
test-agent-verbose: ## 运行 server 测试并显示详细输出
	@echo "$(GREEN)运行 server 测试 (详细输出)...$(NC)"
...
check-agent: ## 检查 server 代码
	@echo "$(GREEN)检查 server 代码...$(NC)"
...
clippy-agent: ## 对 server 运行 clippy
	@echo "$(GREEN)对 server 运行 clippy...$(NC)"
...
run: build ## 运行 server (debug 版本，需要提供参数)
	...
	$(TARGET_DIR)/debug/$(BINARY_NAME) $(ARGS)

run-release: build-release ## 运行 server (release 版本，需要提供参数)
	@echo "$(YELLOW)提示: 需要提供运行参数$(NC)"
	@if [ -z "$(ARGS)" ]; then \
		echo "$(RED)错误: 请提供 ARGS 参数$(NC)"; \
		exit 1; \
	fi
	$(TARGET_DIR)/release/$(BINARY_NAME) $(ARGS)

run-agent: run ## 运行 server (debug)

run-agent-release: run-release ## 运行 server (release)

##@ 初始化

init-config: build ## 生成示例配置文件
	@echo "$(GREEN)生成示例配置文件...$(NC)"
	$(TARGET_DIR)/debug/$(BINARY_NAME) --init
...
install: build-release ## 安装到系统 (需要 root 权限)
	@echo "$(GREEN)安装 server...$(NC)"
	sudo cp $(TARGET_DIR)/release/$(BINARY_NAME) /usr/local/bin/
	@echo "$(GREEN)安装完成: /usr/local/bin/$(BINARY_NAME)$(NC)"

uninstall: ## 从系统卸载
	@echo "$(GREEN)卸载 server...$(NC)"
...
watch: ## 监听文件变化并自动测试 (需要安装 cargo-watch)
	...
	cargo watch -x 'test -p $(PACKAGE)'

watch-check: ## 监听文件变化并自动检查 (需要安装 cargo-watch)
	@echo "$(GREEN)监听文件变化...$(NC)"
	@command -v cargo-watch >/dev/null 2>&1 || { echo "$(RED)错误: 需要安装 cargo-watch$(NC)\n  cargo install cargo-watch"; exit 1; }
	cargo watch -x 'check -p $(PACKAGE)'
